...
<style>
    * { font-family: DejaVu Sans, sans-serif; }
    ...
    h2 { background: #0A2452; color: #fff; font-size: 11px; padding: 4px 8px; margin: 16px 0 6px; text-transform: uppercase; letter-spacing: .5px; page-break-after: avoid; }
...
<div class="wrap">
    ...
    @foreach (['drivers' => ['driver_name','dob','cdl_number','state_issued','experience','date_of_hire'], 'vehicles' => ['year','make','vin','body_type','stated_value'], 'trailers' => ['year','make','vin','body_type','stated_value']] as $rel => $cols)
        @if ($application->$rel->count())
            ...
            <table class="data">
                <thead><tr>@foreach ($cols as $c)<th>{{ __('app.'.$c) }}</th>@endforeach</tr></thead>
                <tbody>
    ...
    @if ($application->coverages->count())
        ...
        <table class="data">
            <thead><tr><th>{{ __('app.coverage') }}</th><th>{{ __('app.limit') }}</th><th>{{ __('app.deductible') }}</th></tr></thead>
            <tbody>
            @foreach ($application->coverages as $c)
                <tr><td>{{ $c->coverage }}</td><td>{{ $c->formatted_limit_amount }}</td><td>{{ $c->formatted_deductible }}</td></tr>
            @endforeach
            </tbody>
        </table>
        </div>
    ...
    <table width="100%" class="sign"><tr>
        <td width="47%">
            @if ($signature)<img src="{{ $signature }}" class="sigimg" alt="client signature">@else<div class="sigimg"></div>@endif
            <div class="sigmeta">{{ __('app.signer_name') }}: {{ $application->signer_name }}</div>
            <div class="sigmeta">{{ __('app.date') }}: {{ optional($application->disclosure_accepted_at)->format('M d, Y') }}</div>
        </td>
        <td width="6%"></td>
        <td width="47%">
            @if ($representativeSignature)<img src="{{ $representativeSignature }}" class="sigimg" alt="legal representative signature">@else<div class="sigimg"></div>@endif
            <div class="sigmeta">{{ __('app.legal_representative') }}: {{ $representativeName }}</div>
            <div class="sigmeta">{{ $representativeTitle }}</div>
        </td>
    </tr></table>
    ...
    <table width="100%"><tr>
        <td class="qrbox">
            ...
            {{ __('app.verify_qr') }}<br>
            {{ __('app.reference') }}: {{ $application->verification_code }}
        </td>
    </tr></table>
    </div>
